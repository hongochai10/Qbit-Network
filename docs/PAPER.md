...
We present QBit Network, a purpose-built blockchain system that replaces all quantum-vulnerable cryptographic primitives with NIST-standardized post-quantum algorithms. QBit Network utilizes ML-DSA-65 (CRYSTALS-Dilithium) for digital signatures and ML-KEM-768 (CRYSTALS-Kyber) for key encapsulation, providing quantum-resistant document notarization, encrypted vault storage, and secure data sharing. The system implements a dual-keypair identity model, Delegated Proof of Stake (dPoS) consensus with epoch rotation and slashing, a domain-separated Merkle tree construction, an EIP-1559 dynamic fee mechanism, and a QBIT token economy (21M max supply, 9 decimals, 5 QBIT block reward with halving). Version 0.7.0 completes the feature set with 11 transaction types, EIP-1559 dynamic fees (base fee adjusts ±12.5% per block, 100% to validator), QBIT balance ledger, epoch reward distribution to delegators, ML-KEM-768 encrypted P2P channels, ctypes-backed secure key material zeroing, TLS auto-provisioning, peer reputation scoring, chain pruning, IPFS integration, a Merkle state trie with verifiable state proofs (stateRoot + receiptsRoot in block headers), a transaction receipt and event system with simple finality, a webhook event delivery system with HMAC-SHA256 signatures, a zero-dependency Python SDK, an OpenAPI 3.0 specification, a NextJS 14 web dashboard with 11 routes, and a legacy single-file SPA chain explorer. Through eighteen rounds of security auditing encompassing 202+ identified vulnerabilities, all of which are resolved, we demonstrate that a production-grade PQC blockchain can be realized with acceptable performance overhead. Our implementation achieves 0.29ms per ML-DSA signature, 3.8ms for block production with 50 transactions, and a per-transaction wire overhead of approximately 10.9KB — a 55x increase over ECDSA-based systems, which we argue is an acceptable trade-off for quantum resistance.
...
5. **A QBIT token economy** with a 21M max supply, EIP-1559 dynamic fee mechanism (base fee adjusts ±12.5% per block), halving block rewards, epoch-based delegator reward distribution, and a double-spend-resistant balance ledger.
6. **A comprehensive security analysis** from seventeen independent audit rounds covering cryptographic, protocol, network, financial, and implementation security, with 0 open findings.
7. **Performance benchmarks** quantifying the overhead of PQC primitives in a blockchain context.
...
QBit Network defines eleven transaction types:
...
**TRANSFER**: Transfers QBIT tokens between addresses. Payload: `{amount: positive integer, memo?: string (max 256 chars)}`. The `to` field must be a valid `qv1` address. Self-transfer is rejected. Balance check includes pending pool debits.
...
| Field | Description |
|-------|-------------|
| `type` | NOTARIZE, STORE, SHARE, REGISTER_KEY, REGISTER_VALIDATOR, REVOKE_KEY, STAKE, DELEGATE, UNSTAKE, EVIDENCE, or TRANSFER |
...
| `baseFee` | EIP-1559 base fee for this block (qubits per unit weight; 0 pre-activation) |
| `signature` | ML-DSA signature over canonical header (includes baseFee) |
...
The system underwent seventeen rounds of security auditing:
...
| 17 | v0.6.0 EIP-1559 + HELLO_AUTH auth bypass + unbonding persistence | 2 |
| **Total** | | **197+** |
...
Prior to v0.3.0, P2P connections were unauthenticated — any node could announce blocks or transactions without proving identity. Version 0.3.0 introduces a 4-step ML-DSA-65 mutual challenge-response handshake (the proof field in step 1 was added in v0.4.0 to resolve SPRINT1-003):

1. **hello_auth** (Initiator → Responder): Initiator sends its ML-DSA public key, derived node address, a 32-byte random challenge, and a self-signed proof (`Sign(sk, AUTH_DOMAIN || challenge || address)`).
2. **auth_response** (Responder → Initiator): Responder verifies the initiator's proof before signing. Then signs the initiator's challenge and issues a counter-challenge.
3. **auth_confirm** (Initiator → Responder): Initiator signs the counter-challenge; both sides verify the other's signature and mark the connection authenticated.
...
| Metric | Value |
|--------|-------|
| Source code (qbit_network/) | ~5,200 lines |
| Test code (tests/) | ~7,800 lines |
| Test cases | 1,358 |
| Test modules | 16 (crypto, wallet, transaction, block, blockchain, adversarial, integration, dPoS, REST API, WebSocket, IPFS, TLS, SecureBytes, reputation, EIP-1559, financial adversarial) |
| Documentation | ~3,500 lines across 10 files |
| Security issues found/fixed | 197+ across 17 audit rounds |
...
│   ├── transaction.py   11 TX types with validation
│   ├── block.py         Block structure with Merkle proofs and baseFee
│   ├── blockchain.py    Chain management, dPoS, epochs, slashing, QBIT balance ledger, pruning
...
The seventeen-round audit process demonstrates the value of iterative security review:
...
- **Round 16** (v0.5.0): Found CRITICAL epoch reward supply inflation (tokens created from nothing), TRANSFER recipient validation gap — fixed. Two LOW findings accepted.
- **Round 17** (v0.6.0): Found CRITICAL HELLO_AUTH auth bypass (timing-dependent proof verification gap) and medium unbonding persistence gap — both fixed.
...
3. **Security requires depth**: 197+ vulnerabilities across 17 audit rounds underscore the complexity of building secure blockchain systems, even with well-studied cryptographic primitives.
...
### Current Status (v0.6.0, 2026-03-26)
...
- QBIT token economy: 21M max supply, 9 decimals, 5 QBIT block reward with halving every 2.1M blocks
- EIP-1559 dynamic fees: base fee adjusts ±12.5% per block, 100% to validator post-activation
- Epoch reward distribution: block rewards accumulated and split to delegators at epoch boundaries
...
- 4-step ML-DSA-65 P2P mutual authentication with verify-before-sign (SPRINT1-003 resolved)
- Genesis validator registered via on-chain REGISTER_VALIDATOR tx (SPRINT1-007 resolved)
- SQLite-primary persistence with staking, epoch, slashing, and balance tables
- 36-endpoint REST API gateway + WebSocket subscriptions + legacy web dashboard + NextJS 14 UI (11 routes)
...
- PROTOCOL_VERSION 3
- 17 audit rounds, 197+ issues found; 1358 tests passing; 0 open issues
...
Coverage: crypto (30+), wallet (19+), transaction (27+), block (14+),
          ...
          dPoS (58), REST API (47+6), WebSocket (34), IPFS (35),
          TLS manager (34), SecureBytes (42), reputation (26),
          pruning (10), proof signature (10),
          EIP-1559 (83+7), financial adversarial (59)
```
