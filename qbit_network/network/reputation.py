...
class PeerReputation:
    ...
    and auto-disconnected. Scores decay toward 0 over time so that old events
    do not permanently affect reputation.
    """
    ...
    DEFAULT_SCORE = 100
    BAN_THRESHOLD = -100
    DECAY_RATE = 0.99  # multiplicative decay toward 0 per decay() call
    ...
    def decay(self) -> None:
        """Apply multiplicative decay to all scores, moving them toward 0.

        Should be called periodically (e.g., once per minute from a cleanup loop).
        Scores decay as: score *= DECAY_RATE
        This ensures old events fade over time.
        """
        to_remove = []
        for peer_id, score in self._scores.items():
            ...
            if abs(new_score) < 0.01 and peer_id not in self._banned:
